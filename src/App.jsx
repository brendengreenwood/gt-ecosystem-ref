...
const PERSONAS = {
  "trading-house-org": {
    ...
    shortTitle: "The House",
    icon: "🏛️",
    tagline: "The machine. Global infrastructure. Sees the whole board.",
    color: "#8B6914",
    accent: "#B8960E",
    role: "You're the ABCD itself — not a person, but the machine. The global infrastructure that connects origins to destinations, that owns the satellites and the vessels and the elevators and the data. You're present in every major producing and consuming region on earth. Your edge is that you see the whole board while everyone else sees their corner.",
    ...
    worldView: "Global. You don't think about corn — you think about calories moving around the planet. US, Brazil, Argentina, Ukraine, Australia on the supply side. China, EU, Middle East, Africa on the demand side. Every origin competes with every other origin. Every destination is an option. You're constantly optimizing the flow.",
    dataAccess: [
      ...
      { type: "proprietary", label: "Satellite & crop modeling", description: "Near real-time imagery across every major growing region. Yield models that beat USDA by weeks. You knew Brazilian safrinha was short before anyone outside the majors." },
      { type: "proprietary", label: "Vessel tracking & trade flows", description: "AIS data on every grain vessel globally. You see China booking Brazilian corn before it shows up in customs data. You know what's on the water." },
      { type: "proprietary", label: "Destination relationships", description: "You're not just selling to importers — you're embedded with them. You know what China is buying before the weekly export sales report." },
    ...
    secondaryActors: ["logistics", "processor", "usda"],
  },
  originator: {
    id: "originator",
    title: "Regional Originator",
    shortTitle: "Originator",
    icon: "🌾",
    tagline: "Covers a territory. Knows the farmers. Hunting deals.",
    color: "#6B8F63",
    accent: "#8AB882",
    role: "You run origination for a 50-mile radius in the Western Corn Belt. You're the pulse — the real-time feel of what's actually happening in the market. Not what the models say. Not what the reports say. What's actually happening with real people making real decisions about real grain.",
    goal: "Capture origination margin by buying grain at favorable basis. Hit volume targets. But more than that: you're the house's sensing network. Your feel for who's selling, who's holding, who's stressed — that's what lets the merchants position ahead of the market.",
    ...
    relationships: [
      {
        partner: "merchant",
        direction: "up",
        dynamic: "The merchant is your boss at the trading house — he's got the global view the house feeds him: satellite data, vessel fixtures, what Brazil is doing, what China is buying. He translates that into your targets. You give him ground truth he can't get from models.",
      ...
      },
      {
        ...
        youProvide: "A bid. Market information. Relationship — you're their window into what the trading house sees. For the farmers who've earned your trust, you might call them first when the house needs bushels fast.",
      },
    ],
    primaryActors: ["producer", "country-elevator", "logistics"],
    secondaryActors: ["processor", "feeder", "lender"],
  },
  merchant: {
    id: "merchant",
    title: "Merchant (Trading House)",
    shortTitle: "Merchant",
    icon: "📡",
    tagline: "The trading house's strategist. Feeds ideas to the field.",
    color: "#B8960E",
    accent: "#D4B42C",
    role: "You're a merchant at one of the ABCDs — the trading house's regional strategist for the Western Corn Belt. You provide deep intelligence for new opportunities. The data infrastructure lets you see patterns and possibilities that no one on the ground could see: where supply is tight globally, where demand is emerging, where the arbitrages are opening up.",
    ...
    worldView: "You think in patterns and relative value. US vs. Brazil vs. Argentina. Gulf vs. PNW. Old crop vs. new crop. Your region is one piece of a global puzzle, and you can see how the pieces fit together. When you spot an opportunity — Brazilian freight spiking, Chinese buying quietly, a processor short on coverage — you can direct your originators to capture it before the market figures it out.",
    dataAccess: [
    ...
    relationships: [
      {
        ...
        youProvide: "Deep intelligence. New opportunities based on real data. The strategic context that tells them where to push and why it matters now.",
      },
      {
        partner: "risk",
        direction: "up",
    ...
    secondaryActors: ["exporter", "processor", "logistics"],
  },
  risk: {
    id: "risk",
    title: "Risk Manager",
    shortTitle: "Risk",
    icon: "📉",
    tagline: "Watches the book. Sets the limits. Sleeps poorly.",
    color: "#8B5A5A",
    accent: "#C47A7A",
    role: "You're risk management at the trading house. You see across all the regional books — every merchant's positions, every originator's commitments. Your job is to make sure the house doesn't blow up. You set position limits, monitor exposure, and have the authority to force liquidation when things get out of hand.",
    goal: "Keep aggregate risk within tolerance. Ensure no single region, commodity, or time period creates existential exposure. Sleep at night knowing you've stress-tested the book against tail scenarios.",
...
function TheHouseView({ isMobile }) {
...
function TradingHouseView({ selectedPersona, onSelectPersona, isMobile }) {
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
...
function ActorCard({ actor, onClick, isActive, isMobile }) {
...
function ActorDetail({ actor, isMobile }) {
  const conf = CONFIDENCE[actor.confidence];
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div style={{ marginBottom: isMobile ? 20 : 28 }}>
...
function SystemDynamicsView({ isMobile }) {
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <h2 style={{
        fontFamily: "'Newsreader', 'DM Serif Display', Georgia, serif",
...
function MobileActorSelector({ selectedActor, onSelect, isOpen, onToggle }) {
